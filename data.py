...
import shutil
import cv2
from cv2 import dnn_superres # OpenCV module for image super-resolution

#vOBJECTIVE:
# The code defines a class, Data_processing, which provides methods for preprocessing and 
# augmenting image and label data. This includes image upscaling, padding, 
# label coordinate transformation, data augmentation, splitting datasets into training, validation, and test sets,
# and preparing data for YOLO (You Only Look Once) model training.
...
class Data_processing():
     ...
     #Fucntion to upscale the image by a factor of 2
     def image_upscaling(self, image):
         sr = dnn_superres.DnnSuperResImpl_create() # Super-resolution model.
         path = "LapSRN_x2.pb" #Pre-trained model file.
         sr.readModel(path)
         sr.setModel("lapsrn", 2) #Sets the model type and scale factor.

         upscaled_image = sr.upsample(image) #upscale the image
         return upscaled_image
     

     #Pads images to a specified window size,
     # placing the original image at a random location within the new padded image.
